...
Prevent the default action of selecting table text */
        ...
        allCells[i].addEventListener("mouseover", function(e){
          if(e.shiftKey) {
            e.target.style.cursor = " url(image/jpf_eraser.png), alias";
          }
          else if(e.altKey) {
            e.target.style.cursor = " url(image/jpf_block.png), cell";
          }
          else {
            e.target.style.cursor = " url(image/jpf_circle.png), pointer";
          }
        });
